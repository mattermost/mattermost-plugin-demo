module github.com/mattermost/mattermost-plugin-demo

go 1.12

require (
	github.com/blang/semver v3.6.1+incompatible
	github.com/mattermost/mattermost-plugin-api v0.0.8
	github.com/mattermost/mattermost-server/v5 v5.3.2-0.20200313113657-e2883bfe5f37
	github.com/pkg/errors v0.9.1
	github.com/reflog/struct2interface v0.3.0 // indirect
	github.com/stretchr/testify v1.5.1
)

replace github.com/mattermost/mattermost-plugin-api => /Users/jesse/Documents/mattermost/src/mattermost-plugin-api
